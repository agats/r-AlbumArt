...
require( [ 'jquery.jsonp-2.1.4.min.js' , 'sprintf-0.7-beta1.js' ], function ( ) {
	...
	albumCoverCollection.saveJson = function ( json ) {
        ...
        // FIXME this should be moved to the caller of this function
			...
			return {
				updateListPane: function ( ) {
					...
					for(i = _currentAlbum; i < albumCoverCollection.length; i++){
                        // FIXME move logic to another function
						(function(index){

							_listPane.append(
								sprintf(
                                    '<li><img src="%(thumbnail)s" acc-id="'+index+'" alt="%(title)s"/>%(title)s</li>',
                                    albumCoverCollection.getAlbum( index )
                                )
							);
